...
web_agent = initialize_agent(
    ...
    verbose=False  # Set verbose to False to suppress debug info
)

# === Enhanced Gemini Summary Prompt ===
...
# === Main Web Query Function ===
def query_web(query: str) -> str:
    """Run a DuckDuckGo web search and summarize the results."""
    try:
        raw_result = web_agent.run(query)
        wrapped_prompt = summary_prompt.format(search_result=raw_result)
        summary = llm.invoke(wrapped_prompt).content.strip()
        return summary  # Only return the final summary without extra info
