...
class ForecastingAgent:
    ...
    def _parse_date(self, date_str: str):
        try:
            ...
            patterns = [
                (r"(?:in\s+)?(\w+)\s+days?\s*(?:after)?", lambda x: today + relativedelta(days=extract_number(x))),
                (r"(?:in\s+)?(\w+)\s+weeks?\s*(?:after)?", lambda x: today + relativedelta(weeks=extract_number(x))),
                (r"(?:in\s+)?(\w+)\s+months?\s*(?:after)?", lambda x: today + relativedelta(months=extract_number(x))),
                (r"(?:in\s+)?(\w+)\s+years?\s*(?:after)?", lambda x: today + relativedelta(years=extract_number(x))),
            ]
...
if __name__ == "__main__":
    ...
    test_queries = [
        "Forecast sales for Nykaa Tea Tree Face Wash in South in two years",
        "Forecast Nykaa Sunscreen SPF 50 in North next week",
        "Forecast for Lavender Body Mist in East tomorrow",
        "Forecast Nykaa Lip Balm in West in five days",
        "Forecast for Rose Face Serum in Central in 3 months",
        "Forecast for Hair Growth Oil in South in six weeks"
    ]
    for q in test_queries:
        print(f"\n🧠 {q}")
        print(agent.forecast(q))
